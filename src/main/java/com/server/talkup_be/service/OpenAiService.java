...
public class OpenAiService {
    ...
    private Map<String, String> createThreadAndRun(String fileId, String extraInfo) {
        ...
        String prompt = (extraInfo == null || extraInfo.trim().isEmpty())
                ...
                : "너는 10년 차 전문 프레젠테이션 컨설턴트이자 대본 첨삭(교정) 전문가야. 사용자가 작성한 기존 대본의 뼈대와 의도를 최대한 존중하면서, 무대에서 완벽하게 발표할 수 있도록 [첨삭 및 윤문]을 진행해 줘. 첨부된 문서는 내용의 사실관계를 확인하거나 빈약한 부분을 채우는 용도로만 사용해.\n\n" +
                "다음 4가지 첨삭 지침을 무조건 엄수해:\n" +
                "1. 원본 텍스트 보존 및 윤문: 완전히 새로운 대본을 창조(생성)하지 마. 사용자가 쓴 원본 문맥을 훼손하지 않는 선에서, 뚝뚝 끊기거나 어색한 문장들을 부드러운 발표용 구어체(~습니다, ~합니다)로 매끄럽게 교정해.\n" +
                "2. 디테일 보강 (살 붙이기): 사용자의 대본 내용 중 논리적 비약이 있거나 설명이 너무 빈약한 부분만 첨부 문서를 참고해서 구체적인 설명이나 예시를 자연스럽게 덧붙여 줘.\n" +
                "3. 발표 리듬감 형성: 청중과 호흡할 수 있도록 문단 사이사이에 적절한 접속사나 환기성 멘트(예: \"자, 그럼\", \"여기서 주목할 점은\", \"다음으로 넘어가서\")를 추가하여 낭독하기 좋은 리듬감을 만들어 줘.\n" +
                "4. 출력 제한사항: '【4:6†material.pdf】'와 같은 출처 주석 기호, 마크다운 특수 기호(###, **), 혹은 '수정된 대본입니다' 같은 인사말은 절대 포함하지 마. 오직 발표자가 바로 인쇄해서 읽을 수 있는 [순수 대본 텍스트]만 출력해.\n\n" +
                "[사용자가 작성한 기존 원본 대본]:\n" + extraInfo;
